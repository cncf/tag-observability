...
* Service Level Indicator (SLI): An SLI is a service level indicator—a carefully defined quantitative measure of some aspect of the level of service that is provided. For example: Error Rate = total_requests / failed_requests
...
* Service Level Agreement (SLA): a business contract that includes consequences of violating the SLO. This is a targeted percentage. For example: 99.9% of requests should be successful. Note this is less then SLO so the SLO is defended (via alerting) prior to the SLA potentially becoming impacted. 
